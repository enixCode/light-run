...
- **Forgot to update the API reference**: the `preversion` hook regenerates and re-stages `website/content/docs/api/` when you run `npm version`, so a version bump cannot ship with stale API docs. If you cut the tag manually without `npm version`, regenerate before tagging: `npm run docs:api && git add website/content/docs/api/ && git commit --amend --no-edit`.
